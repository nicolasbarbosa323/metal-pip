...
chara["Left Arm"].BrickColor = BrickColor.new("Cool Green")
chara["Right Arm"].BrickColor = BrickColor.new("Cool Green")
chara["Left Leg"].BrickColor = BrickColor.new("Cool Green")
chara["Right Leg"].BrickColor = BrickColor.new("Cool Green")
chara.Torso.BrickColor = BrickColor.new("Cool White")
...
function ScatterEff(part)
    ...
    eff1.Size = NumberSequence.new(0.1)
    ...
    eff1.Texture = ""
    eff1.Shape = Enum.ParticleEmitterShape.Box
    eff1.Color = ColorSequence.new(Color3.new(0, 1, 0))

    local eff2 = Instance.new("ParticleEmitter", part)
    eff2.Size = NumberSequence.new(0.1)
    eff2.Transparency = NumberSequence.new({
        NumberSequenceKeypoint.new(0, 0),
        NumberSequenceKeypoint.new(0.9, 0),
        NumberSequenceKeypoint.new(1, 1)
    })
    ...
    eff2.Texture = ""
    eff2.Shape = Enum.ParticleEmitterShape.Box
    eff2.Color = ColorSequence.new(Color3.new(0, 0, 0))
end
...
eff1.Color = ColorSequence.new(Color3.new(0,0,1))
local eff2 = Instance.new("ParticleEmitter",part)
...
eff2.Color = ColorSequence.new(Color3.new(0,0,1))
...
--Animations--
swinganim = chara.Humanoid:LoadAnimation(New("Animation",chara,"Swing",{AnimationId = "rbxassetid://218504594"}))
...
local cooldownTime = 10  -- Cooldown de 10 segundos
local lastUsed = 0

local function createSpike(position)
    local spike = Instance.new("Part")
    spike.Size = Vector3.new(10, 30, 10)  -- Tamanho do espinho
    spike.Shape = Enum.PartType.Wedge
    spike.Anchored = true
    spike.CanCollide = true
    spike.Material = Enum.Material.SmoothPlastic
    spike.Color = Color3.fromRGB(0, 0, 0)  -- Cor preta
    spike.Position = Vector3.new(position.X, 0, position.Z)  -- Colocando no chão
    spike.Parent = game.Workspace

    -- Detecta colisões com outros jogadores
    spike.Touched:Connect(function(hit)
        local character = hit.Parent
        if character:FindFirstChild("Humanoid") and character ~= player.Character then
            -- Perde os humanoids
            local humanoids = character:FindFirstChildOfClass("Humanoid")
            if humanoids then
                -- Se houver dois humanoides, remove ambos
                humanoids.Health = 0
            end
        end
    end)
...
local function spawnSpikes()
    for i = 1, 10 do  -- Cria 10 espinhos
        local randomPosition = Vector3.new(
            math.random(-500, 500),  -- X aleatório
            0,                       -- Colocando no chão (Y = 0)
            math.random(-500, 500)   -- Z aleatório
        )
        createSpike(randomPosition)
    end
end

local function createTextAboveHead()
    -- Verifica se o cooldown foi expirado
    ...
    -- Cria a TextLabel acima da cabeça do jogador
    local textLabel = Instance.new("BillboardGui")
    textLabel.Size = UDim2.new(0, 200, 0, 50)
    textLabel.Adornee = player.Character.Head
    textLabel.StudsOffset = Vector3.new(0, 3, 0)  -- Posiciona um pouco acima da cabeça
    textLabel.Parent = player.Character.Head

    local label = Instance.new("TextLabel")
    label.Size = UDim2.new(1, 0, 1, 0)
    label.BackgroundTransparency = 1
    label.Text = "PREPARE YOURSELF"
    label.TextColor3 = Color3.fromRGB(255, 0, 0)  -- Cor vermelha
    label.TextSize = 24
...
�̈́͆̅̑͌͆͂̅͋̎̄̄̆͗̓͑̿̾̐̑̿̉̃̂͘̕͜Ṗ̵̩̄̽̎͗͛̿̀̀̿̑͒̉̐̄̄͐̈̌̉̕̕͠͝͝S̶̨̢̡̨̛̛̜̤̼̘̭͙͕͙͙̥͈̫͙̼̣̟̀̉̿̓̎̿̊͛̓͊͑͆̇̆̑̿̉́̋̐̀̍̏́̉̆̓͌̓̕͘̕̚̕͜͝͝͝͝E̶̲̱̦͈̩͉͔̹͕̽̐̏̿͌̆̃̔͌̍́͗̏̔̊̉̈̈́͑̀̃͋̃̐̌̎̇͐̽̐̃͑̂͘̕̚̚͘͘͠͝͝͝ͅ"
tecks.FontSize = "Size48"
tecks.TextColor3 = Color3.new(0,0,1)
music1:Stop()
music2:Stop()
...
tecks.Text = "DESISTA"
elseif sel == 3 then
tecks.Text = "HAHAHHAHA"
end
else tecks.Text = "D̷͍̝͇̤̱̝̱̲̼̫͈̜̽̾̿̋̆͒͠͠͝Ḙ̸͎͓̣͕͓̌̈́͠͝V̵̬̱̩͉̯͕̹͕̈Ơ̴͖̦̠͈̯̟̥͈̐̽̑̓̌̀̈́̌̈́̕͠Ŕ̷̰̳͈̝̙̫̱͒͒͝Ư̸͚͇͖͖͓̖̣̜̣̻̪̩͆̏͂͐̄̎̍S̵̝̙͕̬̘̮̺̼͓͍̗̘̑͂̈̀̈́̊̆͘E̵̙̻̥̦̮̜͉̓̂͂̇͋̅͌͒͝C̵̖̺͈̠̜̋L̶̢̛̛̬̗̤͗̃̐̚͝I̷̫̻͔̩̓̑̎́́́͆͑̄̃̐̅´̵̧̤̙̜̣͎͉̳͚͉̆̿̏̓̎̊̆̔̒̏͘S̴̨̢̛̳̰͖̮̜͙̼̮̤̘͇͛͛̈́̈́̈́̓͒̓̊̄̂̇̾̌Ȩ̵̳̥̭͕̺̻̘̫̠̣̰͆̑̍̀̽̋͂̑̋͐̚̚̚͘͜͜"
end
if chara.Humanoid.Health > 0 then
chara.Humanoid.MaxHealth = math.huge
chara.Humanoid.Health = math.huge
game.Lighting.OutdoorAmbient = Color3.new(0,0,1)
...
-- Criar Team "John Doe" em preto
local Teams = game:GetService("Teams")
local player = game.Players.LocalPlayer

-- Criar a Team se não existir
local johnDoeTeam = Teams:FindFirstChild("John Doe") or Instance.new("Team")
johnDoeTeam.Name = "John Doe"
johnDoeTeam.TeamColor = BrickColor.new("Really black")
johnDoeTeam.AutoAssignable = false
johnDoeTeam.Parent = Teams

-- Atribuir jogador à Team John Doe
player.Team = johnDoeTeam

-- Função para mudar team após a morte
local function onDeath()
	wait(1) -- Espera breve pra evitar conflito
	johnDoeTeam.Name = "..."
	player.Team = johnDoeTeam
end
...
-- Conectar imediatamente e quando renascer
connectDeath()
player.CharacterAdded:Connect(connectDeath)



-- Criar luz vermelha forte no jogador
local function addRedLight(character)
...
-- Adiciona a luz quando o personagem spawna
if player.Character then
